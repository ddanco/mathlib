...
an inductive argument on the dimension of the algebra. One begins by choosing either a maximal
...
[as 1937](zorn1937). This also follows from trivially `lie_module.is_nilpotent_iff_forall` simply by
taking `M = L`.

It is pleasing that the two parts of the traditional statements of Engel's theorem are thus unified
into a single statement about nilpotency of Lie modules. It appears that this has not previously
been highlighted.
...
begin
  ...
  { lie_mem' := λ y z hy hz,
    begin
      ...
      exacts [(K.mem_normalizer_iff' x).mp hx₁ v hv, (K.mem_normalizer_iff x).mp hx₁ w hw],
    end,
    .. (R ∙ x) ⊔ (K : submodule R L) },
...
begin
  ...
  { rintros K (hK₁ : lie_algebra.is_engelian R K) hK₂,
    ...
    { apply hK₁,
      ...
      intros x,
      exact module.End.is_nilpotent.mapq _ (h'' x), },
  ...
  { apply is_noetherian_of_surjective L
      (to_endomorphism R L M : L →ₗ[R] module.End R M).range_restrict,
    simp, },
